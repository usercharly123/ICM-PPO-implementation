...
class ICM(nn.Module):
    ...
    def __init__(self, state_dim=172, encoding_size=256, num_layers=2, action_dim=5, activation=nn.Tanh()):
        ...
        # Encoder
        layers = list()
        layers.append(nn.Linear(172, encoding_size))    # WAS 172 !! (so was state_dim)
    ...
    def forward(self, act, curr_obs, next_obs, mask):
        ...
        out = self.fc_i1(torch.cat((curr_enc, next_enc), dim=1))    # WAS dim=2
        out = self.act_i1(out)
        pred_act = torch.transpose(self.fc_i2(out), 0, 1)   # WAS 1, 2
        
        # Augment the dimensions of pred_act and act
        pred_act = pred_act.unsqueeze(0)
        act = act.unsqueeze(0)
        ...
        out = self.fc_f1(torch.cat((one_hot_act.float(), curr_enc), dim=1)) # WAS dim=2
        out = self.act_f1(out)
        ...
        mask = mask.squeeze(0)
        intr_reward = intr_reward.mean(dim=1) * mask    # WAS dim=1
